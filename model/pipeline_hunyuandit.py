...
from diffusers.schedulers import DDPMScheduler, DDIMInverseScheduler
...
from vlm.qwen2_5_vl import Qwen2_5_VLModel
...
class HunyuanDiTPipeline(
    DiffusionPipeline, HunyuanDiTLoraLoaderMixin, SelfReflectionMixin
):
    r"""
    ...
    def __init__(
        ...
        if (
            use_self_reflection is not None
            and isinstance(use_self_reflection, bool)
            and use_self_reflection
        ):
            self.should_skip = False
            self.qwen2_5_vl = Qwen2_5_VLModel(model_path=vl_model_path)
            self.add_noise_scheduler = DDIMInverseScheduler.from_config(
                self.scheduler.config
            )
            self.denoise_scheduler = DDPMScheduler.from_config(
                self.scheduler.config
            )
    ...
    def encode_prompt(
        ...
        if lora_scale is not None and isinstance(self, HunyuanDiTLoraLoaderMixin):
        ...
        if self.text_encoder is not None:
            if isinstance(self, HunyuanDiTLoraLoaderMixin) and USE_PEFT_BACKEND:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.use_self_reflection:
            self.process_steps = kwargs.pop("process_steps", 1)
            self.process_steps_interval = kwargs.pop("process_steps_interval", 5)
            self.process_start = kwargs.pop("process_start", 0.5)
            self.process_end = kwargs.pop("process_end", 0.8)
            self.save_dir = kwargs.pop("save_dir", "./results/test/")
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.use_self_reflection:
                    ...
                    if (
                        (i >= int(num_inference_steps * self.process_start))
                        and (i <= int(num_inference_steps * self.process_end))
                        and i % self.process_steps_interval == 0
